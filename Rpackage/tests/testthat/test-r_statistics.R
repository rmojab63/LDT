...
test_that("AdjustDynamics works", {
  mat <- matrix(c(1:28),7,4)
  colnames(mat) <- paste0("V",c(1:ncol(mat)))
  mat[6,1]=NaN
  mat[7,1]=NaN
  mat[5,2]=NaN
  mat[6,2]=NaN
  ...
  mat[5,4]=NaN
  mat[4,3]=NaN
  res <- AdjustDynamics(mat,TRUE,4,0)
  expected <- matrix(c(1,2,3,4,5,NaN,NaN,
                       NaN,8,9,10,11,NaN,NaN,
                       17,18,19,20,21,NaN,NaN,
                       24,25,26,27,28,NaN,NaN),7,4)
  colnames(expected) <- c("V1","V2(-1)","V3(+2)","V4(+2)")
  expect_equal(expected, res$data)
  expect_equal(c(2L,1L), as.integer(res$hasLags))

  res <- AdjustDynamics(mat,TRUE,2,1)
  expected <- matrix(c(1,2,3,4,5,NaN,NaN,
                       NaN,8,9,10,11,NaN,NaN,
                       15,16,17,18,19,20,NaN,
                       22,23,24,25,26,27,NaN),7,4)
  colnames(expected) <- c("V1","V2(-1)","V3","V4")
  expect_equal(expected, res$data)
  expect_equal(2, res$countNansSet)

  mat = rbind(mat, NaN*numeric(ncol(mat)))
  res <- AdjustDynamics(mat,TRUE,2,3)
  expected <- matrix(c(1,2,3,4,5,NaN,NaN,NaN,
                       NaN,8,9,10,11,NaN,NaN,NaN,
                       NaN,15,16,17,18,19,20,21,
                       NaN,22,23,24,25,26,27,28),8,4)
  colnames(expected) <- c("V1","V2(-1)","V3(-1)","V4(-1)")
  expect_equal(expected, res$data)
