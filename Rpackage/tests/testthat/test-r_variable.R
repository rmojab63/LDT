...
test_that("Bind Variables works with lag/lead adjustments", {
  ...
  mat[5,4]=NaN
  mat[4,3]=NaN
  freq <- F_Monthly(2022,12)

  v1 <- ldt::Variable(mat[,1],"V1",freq, list())
  v2 <- ldt::Variable(mat[,2],"V2",freq, list())
  v3 <- ldt::Variable(mat[,3],"V3",freq, list())
  v4 <- ldt::Variable(mat[,4],"V4",freq, list())

  res <- ldt::BindVariables(list(v1,v2,v3,v4),interpolate = FALSE, adjustLeadLags = FALSE,
                            numExo = 0, horizon = 0)
