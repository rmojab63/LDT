...
search_res <- search.sur(sample$y, sample$x, numTargets = ncol(sample$y),
                         xSizes = c(1:4), 
                         metrics = get.search.metrics(typesIn = c("sic")))
search_res
```
...
search_step_res <-
  search.sur.stepwise(sample$y, sample$x, numTargets = ncol(sample$y),
                      ...
                      metrics = get.search.metrics(typesIn = c("sic", "aic")),
                      items = get.search.items(bestK = 10)
  )
search_step_res
```
...
```{r}
metric_options <- get.search.metrics(typesOut = c("rmse", "crps"), 
...
You may achieve better results by modifying the arguments of the get.search.metrics function and using the `get.search.modelchecks` function to change the default value of `items` (Recall that our goal is to exclude irrelevant `z?` variables and find zero restrictions). One important parameter to consider is the number of simulations, `simFixSize`. Its current value is `r metric_options$simFixSize`. By increasing this value, you can allow the models to demonstrate their capabilities more effectively during the search process. Of course, increasing the `simFixSize` is expensive!
...
Let’s use the sample from the previous example for this purpose. We can use the get.search.items function to request other types of information:

```{r}
search_items <- get.search.items(type1 = TRUE, bestK = 10,
                                 inclusion = TRUE, 
...
search_step_res <-
  search.sur.stepwise(sample$y, sample$x, numTargets = ncol(sample$y),
                      xSizeSteps = x_size_steps, countSteps = count_steps,
                      metrics = get.search.metrics(typesIn = c("aic")),
                      items = search_items)
```
