...
search_step_res <-
  search.bin.stepwise(y = sample_l$y, x = sample_l$x, w = sample_l$w,
                      ...
                      metrics = get.search.metrics(typesIn = c("aic","sic", "auc", "brier")),
...
Usually, there is more than one model in the `summary` output. This is because the output is first “target-variable-specific” and second “evaluation-specific”. In this application, there is just one target but we requested four different types of evaluations in the get.search.metrics function. We can report the results by creating a list of estimated models and using the coefs.table function:
...
```{r}
metric_options <- get.search.metrics(typesOut = c("auc", "brier"), 
