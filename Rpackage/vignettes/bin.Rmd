...
fit_p <- estim.bin(sample_p$y, sample_p$x[,-1], sample_p$w, probType = "probit") 

res_table <- get.coefs.table(list(Logit = fit_l, Probit = fit_p), 
...
Similar to the previous subsection, we can use `get.coefs.table` function and report the results in a table:

```{r}
res_table <- get.coefs.table(list(Logit = fit_l), 
...
One measure (specific to our experiment) is not present in the table. Its formula is $a = \frac{\sum_{i=1}^{n}(p_i-\hat{p}_i)^2}{n}$ where $p_i$ represents the actual probabilities (the green circles in the plot) and $\hat{p}_i$ represents the predicted probabilities (the red plus signs in the plot). This measure is similar to the Brier score but uses actual probabilities. A value of zero indicates a perfect fit and larger values indicate lower explanatory power. The value for this experiment is `r sum((probabilities-predictions)^2)/length(probabilities)`.
...
The `xSizes = c(1:4)` part assumes that we know the number of relevant explanatory variables is less than 5. The `measure_options` part shows that we use SIC measures to evaluate and compare models.
...
search_step_res <-
  search.bin.stepwise(y = sample_l$y, x = sample_l$x, w = sample_l$w,
                      ...
                      measureOptions = get.options.measure(typesIn = c("aic","sic", "auc", "brier")),
...
Usually, there is more than one model in the `summary` output. This is because the output is first “target-variable-specific” and second “evaluation-specific”. In this application, there is just one target but we requested four different types of evaluations in the get.options.measure function. We can report the results by creating a list of estimated models and using the get.coefs.table function:
...
res_table <- get.coefs.table(mod_list, 
                             regInfo = c("obs", "aic", "sic", "aucIn", "brierIn"), 
                             formatLatex = FALSE)
```
...
With current seed and other options, all measures point to a common model. In this regression model a relevant variable (`X3`) is missing and three irrelevant variables are present with non-significant coefficients.

 

## Pseudo out-of-sample simulation

Let’s see if we can do any better by using pseudo out-of-sample evaluations. The following code is similar to the code in the previous section, but we define a pseudo out-of-sample process:

```{r}
measure_options <- get.options.measure(typesOut = c("auc", "brier"), 
...
We use `r measure_options$trainRatio` ratio of the observations (determined by trainRatio) for estimating and the rest for testing. We repeat this experiment `r measure_options$simFixSize` times (determined by `simFixSize`). We can report the result similar to the previous discussion:
