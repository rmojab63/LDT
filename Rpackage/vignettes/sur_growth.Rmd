...
Since **ldt** tries to be an atheoretical tool, we restrict the model set. `ldt::search.sur.stepwise()` function has a step-wise search algorithm. It estimates small regressions, and given a list of criteria, it omits regressors with poor performance. In other words, larger models are not built with variables with relatively low explanatory power in the smaller models.

The steps of `search.sur\_s()` function is defined by a list of indices and a vector of sizes: 
...
searchItems <- get.items.search(model = TRUE, bestK = 50, inclusion = TRUE)
```
...
checkItems <- get.items.modelcheck(maxConditionNumber = 1e15, minDof = 35, minOutSim = 50)
```
...
search_res <- search.sur.stepwise(
  ...
  searchOptions = get.options.search(parallel = TRUE, printMsg = FALSE)
)
```

The arguments are discussed in the previous sections. Note that `search.sur.stepwise()` can save the result of different steps in the working directory. 
...
However, this returned value contains lots of information and we need a way of summarizing them. If the goal is to discuss the coefficients, `ldt::table.coefs()` function can be used to create a table. First, we need to create a list of models:
...
Next, we should choose the types of information from the regressions, we like to display. It gets done based on some keywords from the documentation of the `table.coefs()` function. For example:
...
strategies <- tdata::remove.na.strategies(x_)
```
...
xPcaOptions <- get.options.pca(ignoreFirst = 2, exactCount = 0, max = 8)
```
...
Note that we choose a relatively large value for the type-I error (i.e., `r searchSigMaxProb`). Next, we prepare the data for each case and use `ldt::estim.sur()` function to estimate and create a list of the estimated models:
...
for (ind in inds) {
  ...
  pca_models[[length(pca_models) + 1]] <- estim.sur(y0, x0,
                                                   addIntercept = FALSE,
