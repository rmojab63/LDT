...
searchItems <- get.search.items(model = TRUE, bestK = 50, inclusion = TRUE)
```
...
checkItems <- get.modelcheck.items(maxConditionNumber = 1e15, minDof = 35, minOutSim = 50)
```
...
search_res <- search.sur_s(
  ...
  searchOptions = get.search.options(parallel = TRUE, printMsg = FALSE)
)
```

The arguments are discussed in the previous sections. Note that `search.sur_s()` can save the result of different steps in the working directory. 
...
xPcaOptions <- get.pca.options(ignoreFirst = 2, exactCount = 0, max = 8)
```
