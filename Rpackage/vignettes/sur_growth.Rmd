...
This means that the goal is to explain the long-run GDP per capita growth after `r if (ec) splitYear else NA` by using the information before this year. By information before `r if (ec) splitYear else NA`, we mean the long-run growth rate of all available data, including the GDP per capita itself. 
...
Anyway, we select the @dataset_worldbank0 and automatically create the table of the potential regressors. For this particular data set, we use `ldt::Data_Wdi()` function. Note that for replication, you should download the data set from the WDI website. See the documentation of `Data_Wdi()` function for more information.

Furthermore, since the size of the WDI data set is large and `Data_Wdi()` function is generally time-consuming, it is not a bad idea to save the data at the first step and try to load it from the working directory:

```{r get_data_from_wd_1}
wdi_lr_2005 <- tryCatch(readRDS("data/sur_wdi_2005.RData"), error = function(e) NULL)
```

If it fails (which will happen for the first time), the following code creates and saves the data:

```{r create_data_1}
if (is.null(wdi_lr_2005)) {
...
if (is.null(wdi_lr_2006_21)) {
  ...
  saveRDS(wdi_lr_2006_21, file = "data/sur_wdi_2021.RData")
}
```
...
We remove highly correlated columns and non-country observations from the table of the potential explanatory variables:

```{r data_remove_noncountry}
...
```{r modelset_steps}
xSizes <- list(c(2, 3), c(4), c(5), c(6), c(7:20))
xCounts <- c(NA, 100, 50, 30, 20)
```

This means that at the first step, we use all data and estimate models with 2 or 3 potential regressors; Next, we select `r if (ec) xCounts[[2]] else NA` _best_ potential regressors and estimate models with 4 potential regressors; Next, we select `r if (ec) xCounts[[3]] else NA` _best_ potential regressors and estimate models with 4 potential regressors; and so on.
...
In other words, we are asking **ldt** to use AIC and SIC as the two in-sample criteria, and RMSE and CRPS as the two out-of-sample ones. There are `r if (ec) measureOptions$simFixSize else NA` number of iterations in the simulation and `r if (ec) measureOptions$seed else NA` is used as the seed. In each iteration, `r if (ec) measureOptions$trainRatio*100 else NA` percent of the sample is used for estimation and the rest is used for testing.
...
In other words, we are asking **ldt** to keep the first `r if (ec) searchItems$bestK else NA` best models and also, the inclusion weights.
...
In other words, we are asking **ldt** to omit a regression with a degrees-of-freedom less than `r if (ec) checkItems$minDof else NA`, a condition number larger than `r if (ec) checkItems$maxConditionNumber else NA`, or a number of valid out-of-sample simulations less than `r if (ec) checkItems$minOutSim else NA`. We hope that restricting the condition number can deal with the multicollinearity problem. 
...
search_res <- SurSearch_s(
  ...
  savePre = "data/sur_search_"
)
```

The arguments are discussed in the previous sections. Note that by default, `SurSearch_s()` saves the result of different steps in the working directory. 
...
These numbers are the number of variables in each strategy. While the choice is not straightforward, we choose the first strategy (with `r if (ec) strategies[[1]]$nCols else NA` variables and `r if (ec) strategies[[1]]$nCols*strategies[[1]]$nRows else NA` observation) and the 16th strategy (with `r if (ec) strategies[[16]]$nCols else NA` variables and `r if (ec) strategies[[16]]$nCols * strategies[[16]]$nRows else NA` observation) to fulfill our goal. Of course, since we want a sensitivity analysis on the number of principal components too, we select three values as the thresholds of the cumulative variance ratio. In other words, we use the following combination:

```{r pca_na_strategies_select}
inds <- c(1, 2, 1, 2, 1, 2)
pcaCutoffRates <- c(0.95, 0.95, 0.9, 0.9, 0.6, 0.6)
xPcaOptions <- GetPcaOptions(ignoreFirst = 2, exactCount = 0, max = 15)
```

This means we will estimate 6 models by combining the first two `NA` strategies and three cumulative variance ratios: 0.95, 0.9, and 0.6. The third line of code is just a setup for ignoring the first two regressors and choosing the number of PCs while estimating the model. We restrict the number of PCs to be less than `r if (ec) xPcaOptions$max else NA`. This is a relatively large number and therefore, as the next step, we ask **ldt** to omit insignificant coefficients and re-estimate the model, and repeat this process 2 times:
...
Note that we choose a relatively large value for the type-I error (i.e., `r if (ec) searchSigMaxProb else NA`). Next, we prepare the data for each case and use `ldt::SurEstim()` function to estimate and create a list of the estimated models:
