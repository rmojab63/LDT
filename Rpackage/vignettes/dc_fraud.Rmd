...
```{r data}
vestadata <- tryCatch(readRDS("data/dc_vesta_data.RData"), error = function(e) NULL)
if (is.null(vestadata)) {
  vestadata <- Data_VestaFraud(training = TRUE)
  saveRDS(vestadata, file = "data/dc_vesta_data.RData")
}
```

```{r data_info, echo=FALSE}
sum_vestadata <- sum(lengths(vestadata$data))
sum_vestadata_na <- sum(is.na(vestadata$data))
vestadata_na_perc <- sum_vestadata_na / sum_vestadata * 100
```

In this data set, there are two samples: train and test. We will use the training sample in this vignette. The observations are labeled with _fraud_ or _not fraud_. There are 393 features in the files. Furthermore, each observation has an \textit{id} that can link a part of the observations to another data file with 40 identity-related features. The combined data-set has `r if (ec) ncol(vestadata$data) else NA` features and `r if (ec) round(sum_vestadata/1000000,0) else NA` millions data-points in which `r if (ec) round(vestadata_na_perc,1) else NA`% is `NA`.
