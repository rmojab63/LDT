...
GetSample <- function(n, s) {
  ...
  z <- as.matrix(z)
  x <- z[, c(6, 12, 18)]
  e <- rand.mnormal(n, c(0, 0), s * s * matrix(c(5, 2, 2, 4), 2, 2))
  y <- x %*% t(matrix(c(1, 2, 0, 0, 3, 4), 2, 3, byrow = TRUE)) + e$sample
  colnames(y) <- c("y1", "y2")
...
Given the sample, we use `search.sur.stepwise()` function to find the best model. This function will select one best mode for each of the evaluation methods. We use the results for AIC, SIC, RMSE, and CRPS measures:
...
If you are not familiar with the arguments of the `search.sur.stepwise()` function, please read [this vignette](sur_growth.html) first. The `measureNames` vector is just a display name that we will use later.
...
The `search.sur.stepwise()` function also reports the inclusion weights. The previous function is used to get the score of such a list. In this case, `indices1` and `indices2` are the index of the first 2 explanatory variables in the inclusion list.
...
SampleAndEstimate <- function(i, res, n, s, xSizes, xCounts, yGroups,
  ...
  search_res <- search.sur.stepwise(
    y = sample$y, x = sample$z, numTargets = 2,
    xSizeSteps = xSizes, countSteps = xCounts,
    yGroups = yGroups,
    searchSigMaxIter = 1,
...
for (n in sampleSizes) {
  ...
  for (s in sMultipliers) {
    ...
    for (iter in seq(1, simCount)) {
      ...
      searchItems <- get.items.search(model = TRUE, type1 = FALSE, bestK = 10, inclusion = TRUE)
...
In `search.sur`, `type1` information is the estimated parameters. Note that in order to set a proper value for `cdfs`, we need to run the code (at least) twice. This is because we need to get some general idea about the range of values of estimated coefficients (e.g., by studying the extreme bounds analysis result).
...
```{r}
checkItems <- get.items.modelcheck(maxAic = 10)
```
...
sur_sim_one <- list(
  unr = SampleAndEstimate(
    -1, NULL, n, s, c(3L), c(NA), list(c(1L,2L)), measureOptions,
...
In this example, we present the results for one of the estimated parameters: $\frac{\rho y_1}{\rho x_1}$ in the following figure. Note that the actual value of this parameter is 1.0 (see the equation in the first section). Also, note that we use `ldt::coefs.plot()` and `ldt::s.gld.from.moments()` functions.
