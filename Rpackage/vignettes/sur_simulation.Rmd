...
sampleSizes <- c(30, 100, 1000)
sMultipliers <- c(0.1, 0.5, 1.5, 2)
```
...
```{r estimation_sim}
sur_sim_res <- tryCatch(readRDS("data/sur_sim_res.RData"), error = function(e) NULL)
simCount <- 100
if (is.null(sur_sim_res)) {
  sur_sim_res <- list()
  ...
  saveRDS(sur_sim_res, file = "data/sur_sim_res.RData")
}
```

Note that for each `n`$\in$`sampleSizes` and each `s`$\in$`sMultipliers`, we sample and estimate `r if (ec) simCount else NA` times. This is relatively time-consuming and therefore, we save and try to read data from the working directory.
...
```{r result_compare_measures_model, echo=FALSE, results='hide', fig.show='hold', fig.cap='Scores in finding the true model (n=30, 100, 100)', out.width='45%'}
op <- par(cex = 0.6)
...
sur_sim_one <- tryCatch(readRDS("data/sur_sim_one.RData"), error = function(e) NULL)
if (is.null(sur_sim_one)) {
  sur_sim_one <- list(
  ...
  saveRDS(sur_sim_one, file = "data/sur_sim_one.RData")
}
```

The number of estimated regressions for the unrestricted and restricted model sets are `r if (ec)  sur_sim_one$unr$search_res$counts$searchedCount-sur_sim_one$unr$search_res$counts$failedCount else NA` and `r if (ec) sur_sim_one$res$search_res$counts$searchedCount-sur_sim_one$res$search_res$counts$failedCount else NA`, respectively. 
...
```{r result_compare_measures_inclusion, echo=FALSE, results='hide', fig.cap='Estimated coefficient (searches with and without restricting AIC)', fig.show='hold', out.width="85%"}

intervalMultiplier <- 1.95
use_gld_package <- FALSE

plot_estim <- function(estim, legendSize, intialGld) {
  dat <- as.data.frame(estim, types = c("type1bests"), targets = c("y1"), rows = c("z6"))
  ...
  dat <- as.data.frame(estim, types = c("extremebounds"), targets = c("y1"), rows = c("z6"))
  bounds <- lapply(c(1:4), function(i) list(label = "EB (95%)", xmin = dat$y1.Lower.z6[[i]], xmax = dat$y1.Upper.z6[[i]], col = rgb(0, 0, 1, alpha = 0.4)))

  dat <- as.data.frame(estim, types = c("mixture"), targets = c("y1"), rows = c("z6"))
  ...
  cdfs1 <- sapply(
    ...
    function(i) {
      a <- as.data.frame(estim, types = c("cdf"), cdfIndex = i, targets = c("y1"), rows = c("z6"))
      a$y1.Mean.z6[[1]]
    }
  )
