...
//' Fix Missing Observations and Leads/Lags
//'
//' @param data (numeric matrix) Time-series data with variables in the columns. Endogenous variables comes first.
//' @param numEndo (integer) Number of endogenous variables. The rest is exogenous.
//' @param horizon (integer) Required length of out-of-sample data.
//' It creates lag of exogenous variables or omits \code{NaN}s to make data available.
//'
//' @return (list) results
...
//' \item{data}{(numeric matrix) Final data after the requested fixes.}
...
//'
// [[Rcpp::export]]
List AdjustDynamics(SEXP data, bool interpolate = true, int numEndo = 1,
                    int horizon = 0)
// clang-format on
{
 if (is<NumericMatrix>(data) == false)
   throw std::logic_error("'data' must be a 'numeric matrix'.");
 NumericMatrix data0 = as<NumericMatrix>(data);
 if (numEndo < 0)
   throw std::logic_error("Invalid 'numEndo'. It cannot be negative.");
 if (horizon < 0)
   throw std::logic_error("Invalid 'horizon'. It cannot be negative.");
 auto names0 = colnames(data);
 if (names0 == R_NilValue)
   throw std::logic_error("Invalid 'data'. It must have column names.");
 auto names = clone(as<CharacterVector>(names0));

 auto result = clone(data0);

 auto source = ldt::Matrix<Tv>(&result[0],result.nrow(), result.ncol());
 auto dataset0 = DatasetTs<false>(source.RowsCount, source.ColsCount, true,
                                     false, interpolate, numEndo,
                                     result.ncol() - numEndo, horizon);
 dataset0.Data(source);

 auto ranges = NumericMatrix(dataset0.Ranges.size(), 2);
 for (Ti i=0;i<ranges.nrow();i++){
   ranges(i,0) = dataset0.Ranges.at(i).StartIndex+1;
   ranges(i,1) = dataset0.Ranges.at(i).EndIndex+1;
 }
 colnames(ranges) = wrap(std::vector<const char*>({"Start","End"}));

 auto missingIndices = NumericMatrix(dataset0.WithMissingIndexes.size(), 2);
 for (Ti i=0;i<missingIndices.nrow();i++){
   missingIndices(i,0) = std::get<0>(dataset0.WithMissingIndexes.at(i))+1;
   missingIndices(i,1) = std::get<1>(dataset0.WithMissingIndexes.at(i));
 }
 colnames(missingIndices) = wrap(std::vector<const char*>({"Variable Index","Count"}));

 auto lags = NumericMatrix(dataset0.WithLags.size(), 2);
 for (Ti i=0;i<lags.nrow();i++){
   auto j = std::get<0>(dataset0.WithLags.at(i));
   auto lag = std::get<1>(dataset0.WithLags.at(i));
   lags(i,0) = j+1;
   lags(i,1) = lag;
   names[j] = as<std::string>(names(j)) + std::string("(-") +
     std::to_string(lag) + std::string(")");
 }
 colnames(lags) = wrap(std::vector<const char*>({"Variable Index","Lag"}));

 auto leads = NumericMatrix(dataset0.WithLeads.size(), 2);
 for (Ti i=0;i<leads.nrow();i++){
   auto j = std::get<0>(dataset0.WithLeads.at(i));
   auto lead = std::get<1>(dataset0.WithLeads.at(i));
   leads(i,0) = j+1;
   leads(i,1) = lead;
   names[j] = as<std::string>(names(j)) + std::string("(+") +
     std::to_string(lead) + std::string(")");
 }
 colnames(leads) = wrap(std::vector<const char*>({"Variable Index","Lag"}));

colnames(result) = names;

 auto L = List::create(
   _["hasMissing"] = wrap(dataset0.HasMissingData),
   _["hasMissingIndices"] = wrap(missingIndices),
   _["hasLags"] = lags,
   _["hasLeads"] = leads,
   _["data"] = result,
   _["ranges"] = ranges,
   _["countNansSet"] = wrap(dataset0.CountNanSets)
   );
 return L;
