...
List ConvertTo_Daily(SEXP w, SEXP aggregateFun) {
  ...
  if (aggregateFun == R_NilValue){
    ...
    std::function<double(const std::vector<double>&)> func = [&F](const std::vector<double>& x) { return as<double>(F(wrap(x))); };
    ...
    std::function<double(const std::vector<double>&)> func = [&descType](const std::vector<double>& x) {
      double v;
      Array<double>::GetDescriptive<true>(&x[0],x.size(),descType, v ); //TODO: true as an option?!
      return v;
    };
    v.ConvertTo_Daily(nv, &func);
    ...
    throw std::logic_error("Invalid 'aggregateFun'. It should be a character or a function.");
  }
